# Model genealogy

Generated by `scripts/build_genealogy.py` from `metadata.yaml → parents` only. Do not edit.

```mermaid
graph TD
    seesaw_type1["seesaw_type1<br/>L2"]
    seesaw_type1_2n["seesaw_type1_2n<br/>L0"]
    sm["sm<br/>L3"]
    sm_ckm["sm_ckm<br/>L3"]
    sm_singlet_z2["sm_singlet_z2<br/>L3"]
    thdm_type2["thdm_type2<br/>L3"]
    sm -->|extends| seesaw_type1
    sm -->|extends| seesaw_type1_2n
    sm -->|extends| sm_ckm
    sm -->|extends| sm_singlet_z2
    sm -->|replaces higgs| thdm_type2
```

| id | name | parents | children | maturity |
|---|---|---|---|---|
| `seesaw_type1` | Type-I seesaw (SM + one right-handed neutrino with a Majorana mass) | sm (extends) | — | L2 |
| `seesaw_type1_2n` | Type-I seesaw with three lepton generations and two right-handed neutrinos | sm (extends) | — | L0 |
| `sm` | Standard Model (one Higgs doublet, one full fermion generation) | — | seesaw_type1, seesaw_type1_2n, sm_ckm, sm_singlet_z2, thdm_type2 | L3 |
| `sm_ckm` | Standard Model with three generations and CKM quark mixing | sm (extends) | — | L3 |
| `sm_singlet_z2` | SM + real singlet scalar (Z2-symmetric, spontaneously broken) | sm (extends) | — | L3 |
| `thdm_type2` | Two-Higgs-doublet model, type II, CP-conserving, softly broken Z2 | sm (replaces higgs) | — | L3 |
